...
class DatasetGenerator:
    def build_dataset(self, dataset_name, dl_method, disease, path, augment=False):
        ...
        datasets_available_pneumonia = [name for name in os.listdir(path+"/chexpert") if not name.startswith('.') and not name.endswith('.7z')]
        datasets_avaiable_ards = [name for name in os.listdir(path+"/mimic") if not name.startswith('.') and not name.endswith('.7z')]
        ...
        dataset = Datasets(dataset_name, dl_method, disease, augment, path, path)
        return dataset