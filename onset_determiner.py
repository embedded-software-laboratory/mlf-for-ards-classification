...
class OnsetDeterminer: #class to determine the ARDS onset in a given dataset
    ...
    def set_detection_rule(self, rule):
        if rule not in self.available_detection_rules:
            raise RuntimeError("Detection rule " + rule + " is currently not implemented for ARDS onset detection. Available rules are " + str(self.available_detection_rules))
    ...
    def set_return_rule(self, rule, series_start=None, series_end=None):
        if rule not in self.available_return_rules:
            raise RuntimeError("Return rule " + rule + " is currently not implemented for ARDS onset detection. Available rules are " + str(self.available_return_rules))
        if (rule == "data_series_as_series" or rule == "data_series_as_point") and (series_start == None or series_end == None):
        ...
        for patientid in set(dataframe["patient_id"].to_list()):
            if self.detection_rule == "first_horovitz":
                ...
                if horovitz_index != -1:
                    ...
                    if dataframe["ards"][dataframe[dataframe["patient_id"] == patientid].index[0]] == 0 or self.remove_ards_patients_without_onset == False:
            ...
            if self.detection_rule == "4h_50":
                ...
                for index in dataframe[dataframe["patient_id"] == patientid].index:
                    if dataframe["horovitz"][index] < 300 and dataframe["peep"][index] >= 5:
                        ...
                        while dataframe["patient_id"][second_index] == patientid and dataframe["time"][second_index] - start_timestamp <= 14400:
                            total_number_of_rows += 1
    ...
    def add_return_data(self, return_dataframe, dataframe, index):
        ...
        if self.return_rule == "data_series_as_series":
            ...
            if self.impute_missing_rows == True:
                threshold_timestamp = dataframe["time"][index]
                new_dataframe = self.data_imputator.impute_rows(new_dataframe, threshold_timestamp+self.return_series_start, threshold_timestamp+self.return_series_end)
        ...
        if self.return_rule == "data_series_as_point":
            start_index, end_index = self.get_sub_series_indices(dataframe, index)
            temp_dataframe = pd.DataFrame([[dataframe["patient_id"][index], dataframe["ards"][index], dataframe["time"][index]]], columns = ["patient_id", "ards", "time"])
            new_dataframe = dataframe.iloc[start_index:end_index]
            if self.impute_missing_rows == True:
                threshold_timestamp = dataframe["time"][index]
                new_dataframe = self.data_imputator.impute_rows(new_dataframe, threshold_timestamp+self.return_series_start, threshold_timestamp+self.return_series_end)
            new_dataframe = new_dataframe.drop(["patient_id", "ards", "time"], axis=1) #drop these columns because it makes no sense to have patientid0, patientid1... columns. 
            counter = 0
    ...
    def get_low_horovitz_period_start(self, dataframe, period_length, patientid, return_dataframe):
        ...
        for index in dataframe[dataframe["patient_id"] == patientid].index:
            if dataframe["horovitz"][index] < 300 and dataframe["peep"][index] >= 5:
                ...
                second_index = index
                fulfilled = True #set to false if we find a horovitz above 300 in the next 12 hours - this breaks our ards onset condition
                while dataframe["patient_id"][second_index] == patientid and dataframe["time"][second_index] - start_timestamp <= period_length:
                    if dataframe["patient_id"][second_index] == patientid and dataframe["time"][second_index] - start_timestamp > series_length:
                        series_length = dataframe["patient_id"][second_index] == patientid and dataframe["time"][second_index] - start_timestamp
                        series_start = index
                    if dataframe["horovitz"][second_index] >= 300:
                        fulfilled = False
                        break
    ...
    def get_sub_series_indices(self, dataframe, threshold_index):
        ...
        for i in reversed(range(0, threshold_index)):
            if dataframe["time"][i] - threshold_timestamp >= self.return_series_start and dataframe["patient_id"][i] == patientid:
                start_index = i
            else:
                break
        for i in range(threshold_index, len(dataframe)):
            if dataframe["time"][i] - threshold_timestamp <= self.return_series_end and dataframe["patient_id"][i] == patientid:
                end_index = i
            else:
                break
