...
class DeepAntAnomalyDetector(AnomalyDetector):
    ...
    def _run_step(self, data: dict[str, pd.DataFrame], dataset_to_create: dict[str, Union[list, str]]) -> pd.DataFrame:
        ...
        relevant_train = scaler.transform(relevant_train)
        relevant_val = scaler.transform(relevant_val)
        relevant_test = scaler.transform(relevant_test)
        train_dataset_list = []
        val_dataset_list = []
        test_dataset_list = []
        ...
        for i in range(1, len(train_dataset_list)):
            train_dataset.concatenate(train_dataset_list[i])

        val_dataset = val_dataset_list[0]
        for i in range(1, len(val_dataset_list)):
            val_dataset.concatenate(val_dataset_list[i])

        test_dataset = test_dataset_list[0]
        for i in range(1, len(test_dataset_list)):
            test_dataset.concatenate(test_dataset_list[i])
    ...
    def _build_patient_dict(dataframe: pd.DataFrame) -> dict:
        ...
        for i in range(len(patient_ids)):
            patient_dict[patient_ids[i]] = (int(patient_min_time_df[i]),
